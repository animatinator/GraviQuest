...
class Level():
    def __init__(self, screen, folder, levelnum):
        ...
        self.mainfont = pygame.font.Font("levelfont.ttf", 20)
        self.leveltext = self.mainfont.render("%s, Level %i" % (folder, levelnum), True, (0, 0, 0))
        self.textbacking = pygame.image.load(os.path.join(self.folder, "textbacking.png")).convert_alpha()

        #################################TEMPORARY
        self.cornerpiece = pygame.image.load("cornerpiece.png").convert_alpha()
        #################################TEMPORARY

        self.rotatesound = pygame.mixer.Sound("Rotate.ogg")
        ...
        for y, line in enumerate(leveldata):  # For each row
            ...
            for x, item in enumerate(line):  # For each column in the row
                ...
                if item == "F":  # Floor brick
                    ...
                    thistile.append(None)
                else:  # Tile is either empty (or unimportant) or is a number (used by the tutorial level for InfoPoint placement)
                    try:
                        tileval = int(item)
                        thistile = [tileval, None]
            ...
            if numblocks == 0:
                ...
                # If the block is part of a vertical line
                    ...
                    # If there is a square below
                        ...
                        # If there is a square to the left
                        if (posx > 0) and (self.levelgrid[posy][posx - 1][0] == "F"):
                            block.image = self.floorblimg
                        # If there is a square to the right
                        elif (posx < len(self.levelgrid[0]) - 1) and (self.levelgrid[posy][posx + 1][0] == "F"):
                            block.image = self.floorbrimg
    ...
    def GetSurroundingBlocks(self, block):
        posx = block.rect.left / self.tilesize
        posy = block.rect.top / self.tilesize
        blocks = []
        ...
        if (posx < len(self.levelgrid[posy]) - 1) and (self.levelgrid[posy][posx + 1][0] == "F"):
            blocks.append([posx + 1, posy])
    ...
    def Rotate(self, direction):
        ...
        # Play sound effect
        self.rotatesound.play(0)

    def DrawMap(self):
        surface = pygame.Surface((len(self.levelgrid[0]) * self.tilesize, len(self.levelgrid) * self.tilesize))
        surface.fill((255, 255, 255))

        # Draw floor bricks
        for floorbrick in self.floorbricks.sprites():
            surface.blit(floorbrick.image, floorbrick.rect)
        ...
        # Draw the goal
        surface.blit(self.goalimg, self.goalpos)
        
        # Draw character
        surface.blit(self.character.sprite.images[0][0], self.startpos)
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_q:
                        ...
                        pausemenu  = Menu(self.screen,
                            [["Continue", "continue", None], ["Restart level", "restart", None], ["Save game", "save", [["File 1", 1], ["File 2", 2], ["File 3", 3], ["File 4", 4]]], ["Quit game", "quit", None], ["Take screenshot", "screenie", None]],
                        ...
                        elif selection.find("save") != -1:
                            filename = selection.replace("save", "")

                            infile = open("files.gqs", "r")
                            saves = infile.readlines()
                            infile.close()

                            areanum = self.folder.replace("Area ", "")

                            # Don't add a newline if the item is the last in the file
                            if int(filename) == len(saves):
                                saves[int(filename) - 1] = "%s-%s" % (areanum, self.levelnum)
                            else:
                                saves[int(filename) - 1] = "%s-%s\n" % (areanum, self.levelnum)

                            outfile = open("files.gqs", "w")
                        ...
                        if pygame.key.get_pressed()[pygame.K_DOWN]:
                            mapsurface = self.DrawMap()
                            pygame.image.save(mapsurface, "%s, level %i map.jpg" % (self.folder, self.levelnum))
    ...
    def Update(self):
        ...
        if self.gravitydir == DOWN:
            try:
                if self.levelgrid[chargridpos[1] + 2][chargridpos[0]][0] in solids:
                    canfall = False
            ...
            try:
                if self.levelgrid[chargridpos[1]][chargridpos[0] - 1][0] in solids:
                    if chargridpos[0] > 0:  # Shouldn't be able to stand on the edge of the screen
                        canfall = False
            except:  # The grid position perhaps doesn't exist - assume it is empty
                canfall = True
        elif self.gravitydir == UP:
            try:
                if self.levelgrid[chargridpos[1] - 1][chargridpos[0]][0] in solids:
                    canfall = False
            except:  # The grid position perhaps doesn't exist - assume it is empty
                canfall = True
            if chargridpos[0] < 0:  # Prevents strange errors when the character is offscreen - it should ALWAYS fall, as the offscreen area is empty
            ...
            try:
                if self.levelgrid[chargridpos[1]][chargridpos[0] + 2][0] in solids:
                    canfall = False
            except:  # The grid position perhaps doesn't exist - assume it is empty
                canfall = True
        ...
        if self.character.sprite.rect.colliderect(pygame.Rect(self.goalpos, (50, 50))):
            self.returnval = "win"
    ...
    def Redraw(self):
        ...
        if self.showgrid == True:
            # Draw a grid
            for x in range(0, len(self.levelgrid[0]) + 1):
                xpos = (x*self.tilesize) - self.camerapos[0]
                pygame.draw.line(self.screen, (0, 0, 0), (xpos, 0 - self.camerapos[1]), (xpos, (self.tilesize*len(self.levelgrid)) - self.camerapos[1]))
            for y in range(0, len(self.levelgrid) + 1):
                ypos = (y*self.tilesize) - self.camerapos[1]
                pygame.draw.line(self.screen, (0, 0, 0), (0 - self.camerapos[0], ypos), ((self.tilesize * len(self.levelgrid[0])) - self.camerapos[0], ypos))
            self.needsdrawn = True
        ...
        # Draw the level text and backing
        textpos = ((self.screen.get_width() - self.leveltext.get_width()) - 20, 20)
        centrepos = (textpos[0] + (self.leveltext.get_width() / 2), textpos[1] + (self.leveltext.get_height() / 2))
        backingpos = (centrepos[0] - (self.textbacking.get_width() / 2), centrepos[1] - (self.textbacking.get_height() / 2))
        
        self.screen.blit(self.textbacking, backingpos)
        self.screen.blit(self.leveltext, textpos)

        #################################TEMPORARY
        self.screen.blit(self.cornerpiece, (self.screen.get_width() - self.cornerpiece.get_width(), self.screen.get_height() - self.cornerpiece.get_height()))
        #################################TEMPORARY

        if self.needsdrawn:  # If the level has to be fully redrawn for some reason (eg. new level or gravity change)
            pygame.display.update(self.screen.get_rect())  # Redraw it
            self.needsdrawn = False  # It has now been redrawn
        else:  # Otherwise, redraw only the dirty rects
            pygame.display.update(self.dirtyrects)
        self.dirtyrects = []  # Clear the list
