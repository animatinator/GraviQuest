...
class Character(pygame.sprite.Sprite):
    def __init__(self, parent, pos):
        ...
        self.images = self.LoadImages("character.png")
    ...
    def LoadImages(self, imagefile):
        ...
        for i in range(0, rawimg.get_width() / self.parent.tilesize):
            ...
            strip = pygame.transform.chop(rawimg, rect)
            # Chop off images to the left of the current one
            rect = rawimg.get_rect()
            rect.width = i * self.parent.tilesize
            rect.height = 0
            strip = pygame.transform.chop(strip, rect).convert_alpha()
        ...
        for strip in strips:
            current = []
            for i in range(0, strip.get_height() / (self.parent.tilesize * 2)):
                # Chop off images above the current one
                rect = strip.get_rect()
                rect.width = 0
                rect.height = i * (self.parent.tilesize * 2)
                rect.y = 0
                image = pygame.transform.chop(strip, rect)
                # Chop off images below the current one
                rect = image.get_rect()
                rect.width = 0
                rect.height = rect.height - (self.parent.tilesize * 2)
                rect.y = self.parent.tilesize * 2
                image = pygame.transform.chop(image, rect)
    ...
    def Rotate(self, direction):
        ...
        if temprect != self.rect:
            ...
            if temprect.left < self.rect.left:
                oldrect.left = temprect.left
            else:
                oldrect.left = self.rect.left
            if temprect.top < self.rect.top:
                oldrect.top = temprect.top
            else:
                oldrect.top = self.rect.top
        ...
        for image in self.images:
            ...
            for subimage in image:
                current.append(pygame.transform.rotate(subimage, -(90 * self.rotation)))
            self.rotimages.append(current)
    ...
    def Redraw(self):
        self.image = self.rotimages[self.imagenum][self.animframe / 3 % (len(self.rotimages[self.imagenum]))]
