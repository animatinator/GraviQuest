...
class Level():
    ...
    def Update(self):
        ...
        if self.gravitydir == DOWN:
            try:
                if self.levelgrid[chargridpos[1] + 2][chargridpos[0]][0] not in empties:
                    canfall = False
            ...
            try:
                if self.levelgrid[chargridpos[1]][chargridpos[0] - 1][0] not in empties:
                    canfall = False
            except:  # The grid position perhaps doesn't exist - assume it is empty
                canfall = True
        elif self.gravitydir == UP:
            try:
                if self.levelgrid[chargridpos[1] - 1][chargridpos[0]][0] not in empties:
                    canfall = False
            except:  # The grid position perhaps doesn't exist - assume it is empty
                canfall = True
            if chargridpos[0] < 0:  # Prevents strange errors when the character is offscreen - it should ALWAYS fall, as the offscreen area is empty
            ...
            try:
                if self.levelgrid[chargridpos[1]][chargridpos[0] + 2][0] not in empties:
                    canfall = False
            except:  # The grid position perhaps doesn't exist - assume it is empty
                canfall = True
    ...
    def Redraw(self):
        ...
        # Draw the level text
        self.screen.blit(self.leveltext, ((self.screen.get_width() - self.leveltext.get_width()) - 20, 20))
